...
async def update_user(
    ...
    if user_update.email and user_update.email!=user.email:
        result = await db.execute(select(models.User).where(models.User.emai==user_update.email))
        existing_email = result.scalar().result()
        if existing_email:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
...
async def update_post_full(
    ...
    if post_data.user_id != post.user_id:
        result = await db.execute(select(models.User).where(models.User.id == post_id))
        user = result.scalars().first()
        if not user:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="User not found"
...
async def validation_exception_handler(request: Request, exception: RequestValidationError):
    ...
    return templates.TemplateResponse(
        ...
        "error.html",
        {
            ...
            "message": "Invalid request. Please your input and try again"
        },
        status_code=status.HTTP_422_UNPROCESSABLE_CONTENT
    )